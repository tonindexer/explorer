...
const lastPageFlag = computed(() => store.nextPageFlag(itemCount.value * (pageNum.value+1), 'mint'))
...
const updateValues = async (next: boolean = true) => {
    if (props.keys.length === 0 || pageNum.value === 0)
        await store.loadMintedNFTS(props.minter, itemCount.value, null)
    else {
        await store.loadMintedNFTS(props.minter, itemCount.value, next ? lastLT.value : firstLT.value, itemCount.value * (pageNum.value))
    }
    setExtraFields()
}
