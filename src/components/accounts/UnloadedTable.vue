...
<template>
      ...
      >
        <template v-if="!(acc in specialAccounts)">
          <tr v-if="!isMobile()">
            <td> 
              <NuxtLink
                :to="{ name: 'accounts-hex', params: { hex: acc }, hash: '#overview'}"
        ...
        <template v-else-if="acc in specialAccounts">
          <tr v-if="!isMobile()">
            <td style="text-wrap: nowrap;"> 
              <AtomsAddressField
                :addr="{ hex: specialAccounts[acc].hex, base64: specialAccounts[acc].base64 }"
                :break-word="false"
              />
            </td>
  ...
  <div
    v-if="!hidden"
    class="uk-flex uk-width-1-1 uk-align-left uk-flex-middle"
