...
<template>
  <div
    class="uk-flex status-cell uk-text-primary uk-margin-remove uk-padding-xsmall-vertical uk-padding-small-horizontal"
    :class="status"
    style="gap: 4px; width: fit-content; height: fit-content; border-radius: 8px;"
  >
    <svg
      v-if="status === 'ACTIVE'"
      uk-svg
    ...
    <div
      v-if="status === 'ACTIVE' || status === 'FROZEN' || status === 'UNINIT' || status === 'NON_EXIST'"
      class="uk-margin-remove uk-padding-remove"
    >
      {{ status[0] + status.slice(1,).toLowerCase() }}
    </div>
    <div
      v-else-if="status === 'EXTERNAL_IN' || status === 'EXTERNAL_OUT'"
...
.UNINIT {
    background-color: var(--color-uninit);
    path {
        fill: var(--color-text-emphasis);
    }
}
...
.NON_EXIST {
    background-color: var(--color-nonexist);
    path {
        fill: var(--color-text-emphasis);
    }
}
...
.EXTERNAL_IN, .UPDATE {
    background-color: var(--color-externalin);
}

.EXTERNAL_OUT {
    background-color: var(--color-externalout);
}
