...
<template>
  ...
  >
    <NuxtLink
      ...
      :class="isMobile() ? 'uk-padding-small' : 'uk-padding'"
    >
      <svg
        width="24"
        height="24"
    ...
    >
    <table
      v-if="status !== 'EMPTY' && route.path !== '/search'"
    ...
    >
      <tbody v-if="status === 'FOUND'">
        <tr
          ...
          class="uk-flex uk-flex-column hover"
        >
          <td
            style="max-width: 70vw"
            :style="isMobile() ? 'padding: 0.3rem 10px' : 'padding: 0.3rem 1rem; '"
            @mousedown="goToLink(res)"
          >
            <h4
              v-if="res.type === 'account'"
              class="uk-margin-remove-vertical uk-text-truncate uk-text-primary"
