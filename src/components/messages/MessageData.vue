...
<template>
  <table class="uk-table uk-table-middle uk-table-divider uk-margin-remove-top">
    ...
    <tbody v-if="!isMobile()">
      <template 
        v-for="section of parseData"
        :key="section.title + section.addr"
      >
        <tr v-if="!(section.object && typeof section.value === 'object')">
          ...
          <td
            class="uk-flex uk-width-1-1 uk-flex-middle"
            style="max-width: calc(100vw * 0.75 * 0.75)"
          >
            <AtomsCopyableText
              v-if="!(section.addr && section.value !== 'NONE')"
              :text="section.value.toString()"
            >
              {{ section.value ? section.value : $t('general.empty') }}
            </AtomsCopyableText>
            <NuxtLink
              v-else-if="section.addr"
              class="uk-text-primary uk-text-truncate"
              :to="{name: 'accounts-hex', params: {hex: section.value.toString()}, hash: '#overview'}"
              style="max-width: calc(100vw * 0.75 * 0.7)"
            > 
              {{ section.value }}
            </NuxtLink>
          </td>
        ...
        <template v-else>
          <tr 
            v-for="second of section.value"
            :key="second.addr + second.value"
          >
            <td class="uk-width-1-5">
          ...
          <AtomsPropLine>
            ...
            <template #value>
              ...
              <NuxtLink
                ...
                class="uk-text-primary uk-text-truncate"
                :to="{name: 'accounts-hex', params: {hex: section.value.toString()}, hash: '#overview'}"
                style="max-width: calc(100vw * 0.9)"
              > 
                {{ section.value }}
              </NuxtLink>
            </template>
