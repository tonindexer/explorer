...
<template>
  ...
  <div
    v-if="!loading || data.length > 0"
    class="uk-flex uk-flex-middle uk-margin-small-bottom uk-text-right uk-padding-medium-left"
      ...
      >
        <template v-if="isMobile()">
          <td class="uk-flex uk-flex-column uk-align-center uk-width-1-1 uk-margin-remove-vertical uk-padding-small-vertical uk-padding-medium-horizontal">
            ...
            >
              <AtomsAddressField
                v-if="tline.trader in store.accounts"
                :show-hex="true"
                :break-word="true"
                :addr="composeAddress(tline.trader)"
              />
              <NuxtLink
                v-else
                class="uk-text-emphasis"
                :to="{ name: 'accounts-hex', params: { hex: tline.trader }, hash: '#overview'}"
              >
